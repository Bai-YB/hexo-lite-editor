...
export class PluginWorkerRuntime {
  ...
  request(request: PluginRequest): Promise<unknown> {
    if (this.stopped) return Promise.reject(new Error(uiText("插件停了，重试一次。")));
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => this.terminate(new Error(uiText("插件请求超时了，重试一次。"))), this.timeoutMs);
      ...
      const onError = ((event: ErrorEvent) => this.terminate(event.error ?? new Error(event.message || uiText("插件运行失败，重试一次。")))) as EventListener;
  ...
  terminate(reason = new Error(uiText("插件已停止。"))): void {
