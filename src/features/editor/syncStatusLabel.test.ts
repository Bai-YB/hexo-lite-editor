...
describe("sync status label", () => {
  it("shows actionable human labels and the actual conflict count", () => {
    ...
    expect(syncStatusLabel({ ...view, status: "conflict" })).toBe("2 个文件有冲突");
    expect(syncStatusLabel({ ...view, status: "remoteAhead" })).toBe("云端有更新");
    expect(syncStatusLabel({ ...view, status: "localPending" })).toBe("待同步");
    expect(syncStatusLabel({ ...view, status: "error" })).toBe("同步出错");
