...
- `cargo fmt --manifest-path src-tauri/Cargo.toml -- --check`：通过。Rust 编译、clippy 和完整测试由两端发布构建执行。
- impeccable detector：运行一次，结果为空。截图复核包含两端设置保存/焦点、吸顶和真实编辑的 HTML 排版；控件额外边框已消除。
- 新增交互回归：两引擎共 10 项通过，包括不失焦保存/校验/取消、主题预览及 1360/1120/640px 六组分类和 HTML 折叠/代码显示。
- 完整浏览器矩阵共 116 项已验证通过：初轮 113 项通过；修正 macOS 关闭入口测试、WebKit 设置滚动条宽度变化，以及源码位置往返的亚像素断言后，3 项定向复跑全部通过。源码滚动仍要求同一来源行，往返像素误差限定在 3 CSS px 内。
