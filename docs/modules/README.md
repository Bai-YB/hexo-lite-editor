...
| Markdown 编辑器 | `src/features/editor/EditorPage.svelte`、`MarkdownEditor.svelte` | 文档装载、编辑、保存、文章操作、即时预览和双向滚动 | 页面 → `EditorSessionStore`/`platform`/预览渲染器 |
| HTML 即时预览 | `src/shared/markdown/safeMarkdown.ts` | Markdown/HTML 解析、CSS 限定、DOM 清洗、图片源收集、源码锚点 | 编辑器内容 → 渲染模型 → 安全 HTML 与本地图片解析请求 |
| 全部文件 | `src/features/files`、`src-tauri/src/commands/files.rs` | 项目目录浏览、文本读取保存、冲突和只读限制 | 文件页 → `FileSessionStore` → 文件 IPC |
| 设置与配置 | `src/features/settings`、`src-tauri/src/commands/config.rs` | 六组设置草稿、校验、保存、主题/语言预览和连接设置 | 设置页 → controller → config IPC/同步 IPC |
