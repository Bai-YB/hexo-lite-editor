<div align="center">
  ...
  <p>A quiet, native Windows workspace for Hexo writing, image management, and publishing.</p>
...
- **Truthful image preview** — Markdown and HTML images share the Rust validation pipeline. If a remote image is deleted, has an invalid MIME type, or cannot be revalidated, the preview shows an unavailable state instead of stale bytes.
...
Remote images must use credential-free HTTPS. Redirects, public IP addresses, real MIME bytes, and sizes are validated. Limits are 25 MB per image and 64 MB per batch. Opening or switching articles, returning focus to the app, changing image URLs, or manually refreshing triggers a fresh check, with `no-store` responses and session assets.
